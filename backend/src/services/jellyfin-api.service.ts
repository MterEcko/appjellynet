...
export class JellyfinApiService {
  ...
  constructor(private baseUrl: string) {
    this.api = axios.create({
      ...
      timeout: 10000,
      maxRedirects: 5, // Follow up to 5 redirects (default is 5)
    });
  }
  ...
  async createUser(name: string, password?: string): Promise<JellyfinUser> {
    try {
      // Use Axios with custom config to handle SSL and prevent redirect loops
      const url = `${this.baseUrl}/Users/New`;
      logger.debug(`Creating Jellyfin user at: ${url}`);
      ...
      const response = await axios.post(
        ...
        },
        {
          ...
          httpsAgent,
          maxRedirects: 1, // Allow 1 redirect to get response data
          validateStatus: (status) => status < 400, // Accept 2xx and 3xx as valid
        }
      );

      logger.debug(`Jellyfin createUser response status: ${response.status}`);
      logger.debug(`Jellyfin createUser response data:`, response.data);
      return response.data;
    } catch (error: any) {
      // If we got a redirect response, log it but don't treat as error
      if (error.response && error.response.status >= 300 && error.response.status < 400) {
        const location = error.response.headers.location;
        logger.warn(`Jellyfin returned redirect ${error.response.status} to: ${location}`);

        // For now, treat successful redirect as success
        // The user was likely created even if we got redirected
